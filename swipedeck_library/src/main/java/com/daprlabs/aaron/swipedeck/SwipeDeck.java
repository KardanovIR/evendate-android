...
public class SwipeDeck extends FrameLayout {
    ...
    private void addNextView() {
        if (adapterIndex < mAdapter.getCount()) {
            ...
            if (maskResource != 0) {
                card.setMaskResource(maskResource);
            }
    ...
    private void addLastView(boolean unSwipe) {
    ...
    protected void animateCardPosition(View card, int position) {
        ...
        float scale = (float) (1 - 0.1 * position);
        float alpha = (float) (0 + position * 0.25);
        card.setPivotY(getBottom());
        card.setPivotX((getRight() - getLeft()) / 2);

        View mask = card.findViewById(maskResource);
        mask.animate().alpha(alpha);
        card.animate()
    ...
    public void setMask(int resource) {
    ...
    private void setZTranslations() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            for (int i = 0; i < count; ++i) {
                getChildAt(i).setTranslationZ(i * 1);
            }
        }
    }
